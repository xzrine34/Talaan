...
const users={
"adiaton":"01","bacaycay":"02","broto":"03","caramol":"04","comedia":"05","cuyag":"06","de la cruz":"07","delmoro":"08","delorino":"09","enano":"10",
"esparto":"11","espinola":"12","etac":"13","florano":"14","herreras":"15","jumadiao":"16","loberiano":"17","mangada":"18","paulino":"19","tan":"20",
"velasco":"21","apelo":"22","arceo":"23","arniño":"24","balleta":"25","barojabo":"26","bobiles":"27","caro":"28","cornico":"29","de rafael":"30",
"escalante":"31","frigillana":"32","gallano":"33","gremio":"34","hipe":"35","imperial":"36","irinco":"37","lee":"38","lim":"39","magdaraog":"40","mangada k":"41",
"meregildo":"42","perez":"43","pulga":"44","ponferrada":"45","santos":"46","sidro":"47","sister":"48","teberio":"49","vibar":"50"};
const students=Object.keys(users).map(n=>n.toUpperCase());

let roleType="", loggedStudent="", tardyMinutesData={}, qrStream;
      ...
      html5QrCode.start(cameraId, config, (decodedText) => {
        if(decodedText === studentQRCodes[loggedStudent]){
          html5QrCode.stop().then(() => {
            ...
            setTimeout(()=>{ qrAuthInterface.style.display="none"; main.style.display="block"; studentAction.style.display="block"; loadTable(); loadDataFirebase(); setInterval(updateClock,1000); },500);
          });
...
function cancelQR(){ if(html5QrCode){ html5QrCode.stop().catch(()=>{}); } qrAuthInterface.style.display="none"; login.style.display="block"; }
...
/* ----------------- SEARCH ----------------- */
document.addEventListener("input",function(e){ if(e.target.id==="searchInput"){
  let value=e.target.value.toLowerCase();
  [...tbody.rows].forEach(row=>{ row.style.display=row.cells[1].textContent.toLowerCase().includes(value)?"":"none"; });
}});
...
function markPresent(){
  ...
  let td=row.cells[col]; if(td.textContent!=="") return alert("Already marked");
...
function updateRowSummary(row){
  ...
  for(let i=2;i<row.cells.length-1;i++){ let val=row.cells[i].textContent; if(val==="✔") present++; else if(val==="T") tardy++; else if(val==="C") cutting++; else if(val==="A") absent++; }
...
async function saveDataFirebase(){
  ...
  set(firebaseRef(firebaseDB, `attendance/${weekKey}/students`), attendanceObj);
  set(firebaseRef(firebaseDB, `attendance/${weekKey}/tardy`), tardyMinutesData);
...
async function loadDataFirebase(){
  const weekKey = getWeekKey();
  const snapshot = await get(child(firebaseRef(firebaseDB), `attendance/${weekKey}`));
  if(snapshot.exists()){
...
async function loadWeekOptions(){
  ...
  const snapshot = await get(child(firebaseRef(firebaseDB), `attendance`));
  let keys = [];
  if(snapshot.exists()) keys = Object.keys(snapshot.val());
  keys.sort();
  keys.forEach(k=>{ const option=document.createElement("option"); option.value=k; option.textContent=k.replace("attendance_","Week of "); weekSelect.appendChild(option); });
...
async function loadWeekData(weekKey){
  ...
  if(snapshot.exists()){
    ...
    [...tbody.rows].forEach(row=>{
      ...
      } else { for(let i=2;i<row.cells.length-1;i++){ row.cells[i].textContent=""; row.cells[i].className=""; } }
    });
    updateAllSummaries();
  } else { loadTable(); }
}
...
/* ----------------- MANUAL RESET ----------------- */
function manualReset(){ if(confirm("Reset attendance for this week?")){ const weekKey=getWeekKey(); localStorage.removeItem(weekKey); localStorage.removeItem(`${weekKey}_tardy`); set(firebaseRef(firebaseDB, `attendance/${weekKey}`), null); location.reload(); } }
